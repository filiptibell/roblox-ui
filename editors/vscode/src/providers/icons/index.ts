...
export class IconsProvider implements vscode.Disposable {
	...
	private readonly _onDidChangeCustomIcons: vscode.EventEmitter<void> =
		new vscode.EventEmitter();
	public readonly onDidChangeCustomIcons: vscode.Event<void> =
		this._onDidChangeCustomIcons.event;
	...
	constructor(public readonly providers: Providers) {
		...
		const updateCustomIconDir = () => {
			...
			if (customIconDir && customIconDir.trim().length > 0) {
				...
				runCommand(providers, commandArgs).then(() => {
		...
		this.disposables.push(providers.settings.listen(
	...
	public getClassIcon(className: string): IconPackIcon | undefined {
		const shouldUseNormalIcons = this.customIconsLoading
